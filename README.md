...
## ESL in the Rok pre-analysis (why earlier output was bad)

The earlier scraper accepted almost every `<a>` tag on the page, which pulled navigation and utility labels (for example: `Home`, `Admin`, `View More`) instead of curriculum content. The updated parser now:

- excludes known navigation/noise anchor text,
- excludes utility/admin/auth URLs,
- keeps only links that look curriculum-structured (`textbook`, `unit`, `lesson`, `target sentence`),
- derives `textbook`, `textbook_unit`, and `target_sentence` fields from URL + title.
- fetches each selected curriculum link and extracts sentence-level candidates from semantic tags (`h*`, `p`, `li`, `td`, etc.) before normalization.

This is still a heuristic parser (no full JS rendering), but it now performs two-stage extraction (index links + detail-page sentence mining) and directly addresses the low-quality output shown in your screenshot.
...
This runs the dashboard server inside the container and persists SQLite data to `./data` on your host via a bind mount.
...
### Optional: run tests at container startup

```bash
RUN_TESTS_ON_START=1 docker compose up --build
```
...
docker compose exec esl-scraper-dashboard sh
```

### Build/run without Compose

```bash
docker build -t esl-webscraper:local .
docker run --rm -it -p 8080:8080 -v "$(pwd)/data:/app/data" esl-webscraper:local
```
